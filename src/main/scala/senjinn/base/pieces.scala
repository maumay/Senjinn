...
import senjinn.base.{Square2, SquareSet, Side2, MagicBitboards}
...
trait Moveable
{
   ...
   *  on the board.
   */
  def getControlset(loc: Square2, whites: SquareSet, blacks: SquareSet): SquareSet
   ...
   */
  def getAttackset(loc: Square2, whites: SquareSet, blacks: SquareSet): SquareSet
   ...
   */
  def getMoveset(loc: Square2, whites: SquareSet, blacks: SquareSet): SquareSet
}
...
sealed trait ChessPiece extends Moveable with EnumEntry
{
   ...
   * board.
   */
  def getEmptyBoardMoveset(loc: Square2): SquareSet
  
  /**
   * Get the set of squares this piece controls if it was the only piece on the board.
   */
  def getEmptyBoardControlset(loc: Square2): SquareSet
}
...
object ChessPiece extends Enum[ChessPiece] {
  ...
  def apply(side: Side2): Vector[ChessPiece] = if (side.isWhite) whites else blacks

  
  // Piece implementation
  case object WhitePawn extends ChessPiece {
    val index = 0
    val side = Side2.White
    val shortName = "wp"

    def getControlset(loc: Square2, whites: SquareSet, blacks: SquareSet) = {
      emptyBoardControl(loc.index)
    }

    def getAttackset(loc: Square2, whites: SquareSet, blacks: SquareSet) = {
      getControlset(loc, whites, blacks) & blacks
    }

    def getMoveset(loc: Square2, whites: SquareSet, blacks: SquareSet) = {
      val li = loc.index
    ...
    def getEmptyBoardMoveset(loc: Square2) = {
      emptyBoardMoves(loc.index)
    }

    def getEmptyBoardControlset(loc: Square2) = {
      emptyBoardControl(loc.index)
    }
