...
/**
 * Provides the basic areas on a chessboard as bitboards. More specifically it
 * exposes:
 *  - The ranks and files
 *  - The north-east diagonals and north-west diagonals (anti-diagonals)
 */
object BasicBitboards {
  /**
   * @param index ranges from 0 to 7 inclusive.
   * @return the ith rank where the first rank has index zero.
   */
  ...
  /**
   * @param index ranges from 0 to 7 inclusive.
   * @return the ith file where the h-file has index zero.
   */
  ...
  /**
   * @param index ranges from 0 to 14 inclusive.
   * @return the ith north-eastern diagonal where h1 constitutes the first
   * such diagonal and a8 constitutes the last.
   */
  ...
  /**
   * @param index ranges from 0 to 14 inclusive.
   * @return the ith north-western diagonal where a1 constitutes the first
   * such diagonal and h8 constitutes the last.
   */
  ...
  /**
   * For each square on the board a bitboard is generated by taking all squares
   * in a set of given directions within a certain proximity.
   * @param dirs the set of directions to look in.
   * @param proximity the maximum step distance away from the source square.
   * @return an array of length 64 containing the results for each square.
   */
  def genEmptyBoardBitboards(dirs: Iterable[Dir], proximity: Int = 8): Array[Long] = {
    Square.values.map(sq => sq.allSquares(dirs, proximity)
      .foldLeft(0L)(_ | _.loc)).toArray
  }
...
object MagicBitboards {
  ...
  def getRookControlset(loc: Square, pieces: SquareSet): SquareSet = {
    ...
    val magicIndex = (occupancyvariation * magicnumber) >>> magicshift
    rookMagicMoves(loc.index)(magicIndex.toInt)
  }
  ...
  def getBishControlset(loc: Square, pieces: SquareSet): SquareSet = {
    ...
    val magicIndex = (occupancyvariation * magicnumber) >>> magicshift
    bishMagicMoves(loc.index)(magicIndex.toInt)
  }
  ...
  // Magic bitshifts
  private val bishMagicShifts: Array[Int] = bishOccMasks
    .map(x => 64 - java.lang.Long.bitCount(x))

  private val rookMagicShifts: Array[Int] = rookOccMasks
    .map(x => 64 - java.lang.Long.bitCount(x))
  ...
  private val rookMagicMoves = genMagicMoves(rookOccVars, rookMagicNumbers,
    rookMagicShifts, pmd("r"))
  private val bishMagicMoves = genMagicMoves(bishOccVars, bishMagicNumbers,
    bishMagicShifts, pmd("b"))
  ...
  private def genMagicMoves(c: MagicMoveCons): SquareArr = {
    ...
    (for (i <- 0 to 63) yield {
      ...
      occVars.map(ov => (ov, ((ov * mn) >>> ms).toInt))
        .foreach(p => result(p._2) = calcControlSet(sq, p._1, dirs))
      result
    }).toArray
  }

  private def calcControlSet(sq: Square, occupancyVariation: SquareSet,
                             dirs: Iterable[Dir]): SquareSet = {
